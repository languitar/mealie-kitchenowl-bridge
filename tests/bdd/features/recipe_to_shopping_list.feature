Feature: Push recipe ingredients to a KitchenOwl shopping list
  ...
  Scenario: The ingredient review screen shows each ingredient's quantity from the recipe
    Given a Mealie recipe action is triggered for the recipe "Tomato Soup" with the ingredients:
      | 2 cups | Tomatoes |
    When I select the shopping list "Groceries"
    Then I see the ingredient "Tomatoes" with the quantity "2 cups"

  Scenario: The ingredient review screen shows no quantity for an ingredient without one
    Given a Mealie recipe action is triggered for the recipe "Tomato Soup" with the ingredients:
      |  | Basil |
    When I select the shopping list "Groceries"
  ...
  Scenario: Selecting a shopping list pre-selects a similarly-named existing KitchenOwl item as the match for an ingredient
    ...
    And a Mealie recipe action is triggered for the recipe "Fruit Salad" with the ingredients:
      |  | Bananas |
    When I select the shopping list "Groceries"
