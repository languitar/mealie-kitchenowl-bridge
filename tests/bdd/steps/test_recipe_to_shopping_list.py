...
def recipe_action_triggered_with_quantity(
    running_app, webhook_token, requests_mock, config, recipe_name, ingredient_name, quantity
):
...
def recipe_action_triggered_without_quantity(
    running_app, webhook_token, requests_mock, config, recipe_name, ingredient_name
):
    slug = slugify(recipe_name)
    stub_recipe(
        requests_mock,
